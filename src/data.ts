...
export const resumeData: ResumeData = {
  ...
  projects: [
    ...
    {
      name: 'PeerLink - 基于 WebRTC 的端到端 P2P 即时通讯系统',
      role: '全栈架构师 / 实时通信',
      period: '2026.06 - 至今',
      description:
        '基于 WebRTC 的点对点即时通讯与文件传输系统。文字、语音、文件、实时通话音频与屏幕共享画面在两端浏览器间 P2P 直传，不经过服务器（服务端仅做信令撮合，看不到任何内容）。支持多会话并行房间、局域网自动发现与跨网络扫码配对。',
      techStack: [
        'React 19',
        'Vite',
        'TypeScript',
      ...
      difficulties: [
        'WebRTC P2P 全链路：封装 RTCPeerConnection / ICE / DataChannel 建立，单条 DataChannel 多路复用「文字 + 语音消息 + 文件传输 + 通话控制 + 屏幕共享控制」，按 transferId / fileId / msgId / callId 路由；信令与传输彻底分离，业务数据永不经过服务器。',
        '实时通话与屏幕共享状态机：实现电话式振铃、双向音频轨实时传输、1v1 会议 + 屏幕共享视频轨直传；固定 initiator 端发起 renegotiation 规避 glare，ICE 短暂断连设宽限期自愈，掉线会话灰显保留可重连。',
        '多会话并行房间架构：SessionManager 持有 N 条彼此隔离的会话（各自独立 ws + RTCPeerConnection + DataChannel + Conversation 编排器），信令层无需感知多会话，复杂度全部收敛在前端 state 层，DataChannel 严格保持 1-1。',
        '文件传输工程：自研分片传输 + 背压控制 + CRC32 校验 + 组装，支持 GB 级大文件流式落盘（不全攒内存）与整个文件夹保留目录结构传输；接收端按能力探测在 File System Access 与内存 Blob 间切换。',
        '协议层单一事实源：packages/protocol 以 zod discriminated union 定义全部信令消息、控制帧与分片帧，前后端共享同一套类型与运行时校验；信令服务全内存（无数据库），按出口 IP 分组实现局域网设备自动发现。',
        '工程化：pnpm workspace + Turborepo 组织 web / signaling / protocol 多包；catalog 集中版本声明，Docker 多镜像 + Traefik 反代 + GitHub Actions（lint / typecheck / test）CI。',
      ],
      highlights: [],
      url: 'https://peerlink.qinjiapeng.com',
      github: 'https://github.com/qjp88995/peerlink',
      achievements:
        '在纯浏览器环境实现服务端零内容感知的端到端通信链路，沉淀可复用的协议层与传输内核，为后续原生 App 与小程序阶段复用同一套信令服务与协议包奠定基础。',
    },
    {
