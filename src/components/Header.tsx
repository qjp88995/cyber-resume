...
export const Header = ({ data }: { data: ResumeData }) => {
  ...
  useGSAP(() => {
    // Initial drop-in and fade for header
    gsap.from(headerRef.current, {
      ...
      duration: 0.8,
      ease: "expo.out",
      clearProps: "all", // Clear inline styles after animation so hover transforms work
      onComplete: () => {
        // Add transition classes AFTER animation completes to avoid CSS/GSAP conflict
        headerRef.current?.classList.add("transition-all", "duration-300");
      },
    });

    // Blinking system text
  ...
  return (
    ...
    >
      ...
      <div className="text-xl text-text-dim mb-8 print:text-gray-800 print:mb-2 print:text-lg">
        {data.title} | 10 YEARS EXP
      </div>
